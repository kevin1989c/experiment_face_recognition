...
def use_CNN(train_i,datacsv_file,filter_num1, filter_num2,filter_h,filter_w,img_h,img_w,poolsize,input_feature,total_image_num,person_num, count_incorrect=0.0): 
    ...
    layer3 = LogisticRegression(input=layer2.output, params_W=layer3_params[0],params_b=layer3_params[1],n_in=200, n_out=person_num)  
...
if __name__ == '__main__':
      f_handler=open('CNN_recog_rate_sheffield.log', 'w')
      sys.stdout=f_handler
      for train_i in range(4,14):
        print('face image trainning number%i'%int(train_i))
	use_CNN(train_i,
        datacsv_file='sheffield_side.csv',
        filter_num1=15,
        filter_num2=30,
        filter_h=5,
        filter_w=5,
